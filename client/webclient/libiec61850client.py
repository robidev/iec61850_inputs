	...
	if dataAttributes != None:
		...
		while dataAttribute != None:
			...
			if submodel:
				...
				if error == 0:
					model[daRef]['value'], model[daRef]['type'] = printValue(value)
...
def writeValue(con, model, ref, value):
	...
	mmsvalue = getMMsValue(model[ref]['type'],value)
	if not mmsvalue:
		return model,-1
	...
	if error == 0:
		...
		if error == 0:
			model[ref]['value'], model[ref]['type'] = printValue(RetValue)
...
def updateValueInModel(con, model, ref):
	if model[ref]['reftype'] != 'DA': 
		error = -1
		submodel = printDataDirectory(con,ref)
		if submodel:
			model.update(submodel)
			error = 0
	else:
...
def registerWriteValue(ref, value):
	global polling
	global connections
	...
	err = getIED(uri_ref.hostname, port)
	if err == 0:
		...
		if uri_ref.path[1:] in model:
			model, error = writeValue(con, model, uri_ref.path[1:], value)
			if error == 0:
...
def poll():
	...
	for key in polling:
		...
		model = connections[tupl]['model']
		if con and model:
			model, err = updateValueInModel(con, model, uri_ref.path[1:])
			if err == 0:
				connections[tupl]['model'] = model
				#emit value to webclient
				print("value:%s" % model[uri_ref.path[1:]]['value'])
			else:
...
if __name__=="__main__":
	...
	err = registerReadValue("iec61850://127.0.0.1:102/TEMPLATEMUnn/TVTR1.Vol.instMag.i")
	print("registered 1")
	err = registerReadValue("iec61850://127.0.0.1:102/TEMPLATEMUnn/TVTR2.Vol.instMag.i")
	print("registered 2")
	err = registerWriteValue("iec61850://127.0.0.1/TEMPLATEMUnn/LLN0.MSVCB01.SvEna",True)
	while True:
		poll()
		#print("poll")
		time.sleep(0.719)
		registerWriteValue("iec61850://127.0.0.1:102/TEMPLATEMUnn/LLN0.MSVCB01.SvEna",False)
